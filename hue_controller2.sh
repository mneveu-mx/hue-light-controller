...
enrich_rgb() {
	cptmp "$1"
	enrich_json_with_arg "$1" "state" "rgb" "value"
	finalize "$1"
}

enrich_rgbe() {
	cptmp "$1"
	enrich_json_with_arg "$1" "state" "rgbe" "value"
	finalize "$1"
}
...
do
	...
	then
		log "Enrich datas for lights"
		for entry in "$PATH_STORE_LIGHTS"/*
		do
			#Copy $PATH_STORE_LIGHTS to temp file
			## TODO
			cptmp $entry
			# Enrich
			##TODO
			## 
			enrich_rgb "$entry.tmp"
			enrich_rgbe "$entry.tmp"
			##
			#Finalize file
			finalize $entry
		done
